...
def get_progress(model_type, scenario_index, min_estimators, max_estimators, step_size, num_shuffle_iterations=1, is_recurrent=False, verbose=False):
...
def get_progress(model_type, scenario_index, min_estimators, max_estimators, step_size, num_shuffle_iterations=1, is_recurrent=False, verbose=False, target='T'):
    model = get_model(model_type)
    if model_type == "RandomForest" and num_shuffle_iterations == 1:
        model.set_params(warm_start=True)
    ...
    for i in range(min_estimators, max_estimators + 1, step_size):
        ...
        for j in range(num_shuffle_iterations):
            ...
            X_train, y_train = get_train_data_sulfur(scenario_index, train_index, is_recurrent, target)
